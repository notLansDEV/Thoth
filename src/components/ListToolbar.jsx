      ...
      <div style={{ position: 'relative' }}>
        ...
        {open && (
          ...
          >
            {options ? (
              <>
                <label style={{ display: 'block', fontSize: '10px', fontWeight: 700, color: 'var(--muted)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                  Action
                </label>
                <select
                  value={filterValue}
                  onChange={(e) => onFilter(e.target.value)}
                  style={{ background: 'var(--panel3)', border: '1px solid var(--border)', color: 'var(--text-soft)', borderRadius: '4px', fontSize: '11px', padding: '5px 7px', width: '100%' }}
                >
                  {options.map((o) => (
